...
const SnackbarSetProvider = React.memo<TSnackbarSetProps>(({ setSnackbar, children }) => {
  const handleSnackbarSet = (open = false, message: string, type = 'success' as Color) => {
    setSnackbar({
      message,
      type,
      open,
...
export default function SnackbarProvider({ children }: TProps): React.ReactElement {
  ...
  return (
    <SnackbarValueContext.Provider value={snackbar}>
      <SnackbarSetProvider setSnackbar={setSnackbar}>{children}</SnackbarSetProvider>
